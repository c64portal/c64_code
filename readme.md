
# C64 Code repository

This repo contains code for my various Commodore 64 releases - mainly demoscene.
Waht you can find here so far:

- MD Invite Intro - C (kickC) source code and resources of the small invitation intro for Moonshine Dragons C64 party
- Censor Intro - censor_intro/readme.md -  C (Oscar64) src code for Censor Design intro I made for a compo

# Purpose and copyrights 

This source code is realesed for educational purposes of how to use KickC Oscar64 (mixed with some assembly code) to create little C64 productions.


# Licensing details:

This release is using Creative Commons licensing. Attribution-NonCommercial-NoDerivatives 4.0 International (CC BY-NC-ND 4.0)

Summary can be found here: https://creativecommons.org/licenses/by-nc-nd/4.0/

Details can be found here: https://creativecommons.org/licenses/by-nc-nd/4.0/legalcode